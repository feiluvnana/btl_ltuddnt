...
class AuthenBloc extends Bloc<AuthenEvent, AuthenState> {
  AuthenBloc() : super(const AuthenState()) {
    on<AuthenLogin>((event, emit) async {
      await Api().login(event.email, event.password).then((value) {
        if (value == null) {
          ...
          secureStorage.write(key: "user", value: jsonEncode(value["data"]));
          emit(state.copyWith(user: User.fromJson(value["data"])));
        }
    ...
    on<AuthenChangePassword>((event, emit) async {
      await Api().changePassword(event.password, event.newPassword).then((value) {
        if (value == null) {
          Fluttertoast.showToast(msg: "Có lỗi với máy chủ. Hãy thử lại sau.");
        } else if (value["code"] != "1000") {
          Fluttertoast.showToast(msg: resCode[value["code"]] ?? "Lỗi không xác định.");
    ...
    on<AuthenGetVerifyCode>(((event, emit) async {
      await Api().getVerifyCode(event.email).then((value) {
        if (value == null) {
          Fluttertoast.showToast(msg: "Có lỗi với máy chủ. Hãy thử lại sau.");
        } else if (value["code"] != "1000") {
          Fluttertoast.showToast(msg: resCode[value["code"]] ?? "Lỗi không xác định.");
